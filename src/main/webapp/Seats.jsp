    ...
    <body>
        ...
        <script>
            ...
            document.addEventListener("DOMContentLoaded", function () {
                const datePicker = flatpickr("#selectedDateInput", {
                    dateFormat: "Y-m-d", // Format as yyyy-mm-dd
                    defaultDate: "today",
                    onClose: function (selectedDates, dateStr) {
                        selectedDate = dateStr; // Update the selected date
                        fetchData();

                    }
                });

                // Function to trigger the date picker programmatically
            ...
            function fetchData() {
                ...
                // Make a GET request to the servlet with the selected date as a parameter
                fetch("RetrieveTicketsByDateServlet?selectedDate=" + selectedDate)
                        ...
                        .then(data => {
                            ...
                            let combinedData = "";
                            const seatSet = new Set(data); // Convert the array to a Set for efficient lookup
                            for (let i = 1; i <= 40; i++) {
                                const seatId = "S" + i;
                                const isBooked = seatSet.has(seatId) ? "booked" : ""; // Use 'has' for set lookup
...
<body>
    <h2>Fetch Ticket Booking Data</h2>

    <!-- Input to Store the Selected Date -->
    <input type="text" id="selectedDateInput" placeholder="Select Date" readonly>
